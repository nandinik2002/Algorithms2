    ...
    public int countWords(char[][] boggleBoard, DictInterface dictionary) {
     ...
     for (int i = 0; i < boggleBoard.length; i++) {
         for (int j = 0; j < boggleBoard[i].length; j++) {
             count += dfsCountWords(i, j, boggleBoard, dictionary, word, visited);
         }
     }
     return count;
}
...
private int dfsCountWords(int i, int j, char[][] board, DictInterface dict, StringBuilder word, boolean[][] visited) {
    if (i < 0 || i >= board.length || j < 0 || j >= board[0].length || visited[i][j]) return 0;
    char currentChar = Character.toLowerCase(board[i][j]);
    word.append(currentChar);
    visited[i][j] = true;

    int count = 0;
    int result = dict.searchPrefix(word);
    if (result == 2 || result == 3) count = 1; // Found a valid word
    if (result > 0) { // Continue DFS if prefix or valid word
        for (int di = -1; di <= 1; di++) {
            for (int dj = -1; dj <= 1; dj++) {
                if (di != 0 || dj != 0) count += dfsCountWords(i + di, j + dj, board, dict, word, visited);
            }
        }
    }

    word.setLength(word.length() - 1); // Backtrack
    visited[i][j] = false;
    return count;
}
  ...
  private boolean dfsFindAnyWord(int i, int j, char[][] board, DictInterface dict, StringBuilder word, boolean[][] visited) {
   ...
   private boolean dfsFindAnyWordOfLength(int i, int j, char[][] board, DictInterface dict, StringBuilder word, boolean[][] visited, int length) {
    if (word.length() == length) {
        int result = dict.searchPrefix(word);
        return (result == 2 || result == 3); // Check if it's a valid word at this length
    }
